...
- **Logo Header:**
  - **Background Terang (Light Background):** Menggunakan logo `Logo1.png` (versi warna Deep Maroon & Orange asli).
  - **Background Gelap (Dark Background / Real Image Cover):** Menggunakan logo `ubp.png` (versi logo putih / kontras terang).
  - Logo diletakkan di sudut kiri atas dengan `padding-top: 30px` dari batas paling atas.
...
1. **Pembuatan Desain (Tugas Antigravity):**
   - Membuat/mengedit file `index.html` lengkap (Slide 1 s/d Slide 5) dengan aset ter-embed Base64 Data URI.
   - Menyediakan tombol *"Download Semua Slide (PNG)"* di bagian atas kanvas HTML.
   - **Tanpa Auto-Render AI (Hemat Token):** AI **TIDAK MELAKUKAN** render otomatis atau penyimpanan gambar ke folder lokal (`render_slides/`). Ini menghemat kuota token dan mempercepat waktu pengerjaan.
...
3. **Proses Download & Unggah ke Instagram (Tugas Pengguna - Manual):**
   - Pengguna membuka `index.html` di browser lokal dan mengunduh file gambar PNG secara manual menggunakan tombol download di kanvas HTML.
   - Pengguna mengunggah file PNG ke Instagram secara manual serta menyalin draf caption & hashtag yang telah disiapkan oleh AI.
